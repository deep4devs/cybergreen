...
export const getIsraelCyberNotices = async (lang: Language): Promise<ResourceNotice[]> => {
  ...
  const response = await ai.models.generateContent({
    model: "gemini-3-pro-preview",
    contents: `List 4 real-world, recent cybersecurity alerts from prominent Israeli firms (Check Point, Wiz, SentinelOne, CyberArk). Language: ${isEs ? 'SPANISH' : 'ENGLISH'}.`,
    config: {
      tools: [{ googleSearch: {} }],
      responseMimeType: "application/json",
      responseSchema: {
