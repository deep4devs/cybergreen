  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 ${isScrolled ? 'py-4' : 'py-6'}`}>
      <div className="max-w-7xl mx-auto px-6">
        <div className={`flex items-center justify-between px-6 py-3 rounded-2xl transition-all duration-500 ${isScrolled ? 'glass' : 'bg-transparent'}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="bg-white text-black px-5 py-2 rounded-xl text-[11px] font-extrabold uppercase tracking-wider hover:bg-emerald-500 hover:text-white transition-all">{t.login || 'Login'}</button>
          </div>
        </div>
      </div>
    </nav>
